...
**Reham-only policy**: `colorscheme.lua` also disables the two themes LazyVim core bundles (`folke/tokyonight.nvim`, `catppuccin/nvim`) so they are pruned on `Lazy! sync`, and `loader.lua` uses `install.colorscheme = { "reham_mist", "habamax" }`. Neovim's own runtime still ships built-in themes (`catppuccin.vim`, `habamax` included in this Nix build) — those cannot be removed and are simply never selected by the config.
