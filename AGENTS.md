...
**Reham-only policy**: `colorscheme.lua` also disables the two themes LazyVim core bundles (`folke/tokyonight.nvim`, `catppuccin/nvim`) so they are pruned on `Lazy! sync`, and `loader.lua` uses `install.colorscheme = { "reham_mist" }`. Neovim's own runtime always ships ~28 built-in themes (`blue.vim`, `catppuccin.vim`, `habamax.vim`, ...); they cannot be removed from the installed package, so `colors/` contains **shadow files** with the same names that `:echoerr` and refuse to load — `:colorscheme habamax` etc. fails and the previous theme stays. **Exception: `default.vim` is never shadowed** — it is Neovim's baseline theme and plugins restore to it internally (e.g. `snacks.nvim` picker preview saves/restores via `vim.g.colors_name or "default"`), so erroring on it breaks them. If Neovim ever adds new built-in themes, mirror their filenames in `colors/` (`.vim` for vim, `.lua` for lua).
