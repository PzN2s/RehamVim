...
### Bug Delta (feature)
`lua/boot/bugdelta.lua` (loaded last in `loader.lua`) diffs diagnostics after a save: `BufWritePre` snapshots the current buffer's diagnostics, `BufWritePost` starts an adaptive settle (polling at 400/1100/2200/3800 ms) so slow LSP re-analysis isn't missed, then reports newly introduced errors/warnings via `Snacks.notify` and a lualine `Δ+N`/`Δ0` marker (`vim.g.reham_bugdelta`). Commands: `:RehamBugDelta` (analyze now — always replies, even establishing a baseline on first use), `:RehamBugDeltaToggle` (on/off auto), `:RehamBugDeltaList` (loclist; falls back to the file's current errors/warnings if nothing is new). Do NOT let `M.run` silently advance the baseline before diagnostics settle.
