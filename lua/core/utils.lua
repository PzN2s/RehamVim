...
local function substitute(cmd)
  ...
  local result = cmd:gsub("%%", file)
  result = result:gsub("$fileBase", file_base)
  result = result:gsub("$filePath", file_path)
  return result
end
...
local function bootstrap_project()
  local categories = {
    {
      name = "  Frontend",
      items = {
  ...
  local function run_in_terminal(dir, cmd, msg)
    ...
    local terminal_cmd = string.format("cd %s && %s", dir, cmd)
    if vim.fn.has("win32") == 1 then
      terminal_cmd = string.format("cd %s ; %s", dir, cmd)
    end
    Snacks.terminal.open(terminal_cmd, {
      cwd = dir,
      win = {
        position = "float",
        border = "rounded",
    ...
    if selected.name:find "Laravel" then
      local kits = {
        ...
        },
        {
          ...
          cmd = "composer require laravel/jetstream && php artisan inertia --stack=react",
        },
        {
